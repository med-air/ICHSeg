...
from nnunetv2.training.nnUNetTrainer.variants.network_architecture.network_rank import PlainConvUNet
from nnunetv2.training.nnUNetTrainer.nnUNetTrainer import nnUNetTrainer
class nnUNetTrainer_rank(nnUNetTrainer):
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            output, ms1, ms2, ms3 = self.network(data, True)
            pred = torch.nn.functional.softmax(output[0].detach(), dim=1)[:,1:, :,:,:]

            seg = target[0]
            seg_patch = seg.reshape(-1, 1, 4, 4, 5, 64, 5, 64) \
                .permute(0, 1, 2, 4, 6, 3, 5, 7).reshape(-1, 1, 4, 64, 64)
            score = seg_patch.reshape(seg_patch.shape[0], -1).sum(dim=1).flatten()
            pred_patch = pred.reshape(-1, 1, 4, 4, 5, 64, 5, 64) \
                .permute(0, 1, 2, 4, 6, 3, 5, 7).reshape(-1, 1, 4, 64, 64)
            pred_score = pred_patch.reshape(pred_patch.shape[0], -1).sum(dim=1).flatten()
            idx_pos = torch.where(score> 0)[0].detach().cpu().numpy()
            idx_neg = torch.where(score == 0)[0].detach().cpu().numpy()
            idx_neg_rank = torch.argsort(pred_score[idx_neg], descending=True).detach().cpu().numpy()
            idx_neg = idx_neg[idx_neg_rank][:int(0.2 * len(idx_neg))]
            idx_pos = np.random.permutation(idx_pos)
            idx_neg = np.random.permutation(idx_neg)
            if len(idx_pos) > len(idx_neg):
                idx_pos = idx_pos[:len(idx_neg)]
            elif len(idx_pos) < len(idx_neg):
                idx_neg = idx_neg[:len(idx_pos)]
            ms1 = ms1.flatten()
            ms1 = ms1[idx_pos]-ms1[idx_neg]
            labels1 = torch.ones_like(ms1)

            seg_patch = seg.reshape(-1, 1, 4, 4, 10, 32, 10, 32) \
                .permute(0, 1, 2, 4, 6, 3, 5, 7).reshape(-1, 1, 4, 32, 32)
            score = seg_patch.reshape(seg_patch.shape[0], -1).sum(dim=1)
            pred_patch = pred.reshape(-1, 1, 4, 4, 10, 32, 10, 32) \
                .permute(0, 1, 2, 4, 6, 3, 5, 7).reshape(-1, 1, 4, 32, 32)
            pred_score = pred_patch.reshape(pred_patch.shape[0], -1).sum(dim=1).flatten()
            idx_pos = torch.where(score> 0)[0].detach().cpu().numpy()
            idx_neg = torch.where(score == 0)[0].detach().cpu().numpy()
            ...
            if len(idx_pos) > len(idx_neg):
                ...
                idx_neg = idx_neg[:len(idx_pos)]
            ms2 = ms2.flatten()
            ms2 = (ms2[idx_pos] - ms2[idx_neg])
            labels2 = torch.ones_like(ms2)

            seg_patch = seg.reshape(-1, 1, 8, 2, 20, 16, 20, 16) \
                .permute(0, 1, 2, 4, 6, 3, 5, 7).reshape(-1, 1, 2, 16, 16)
            score = seg_patch.reshape(seg_patch.shape[0], -1).sum(dim=1)
            pred_patch = pred.reshape(-1, 1, 8, 2, 20, 16, 20, 16) \
                .permute(0, 1, 2, 4, 6, 3, 5, 7).reshape(-1, 1, 2, 16, 16)
            pred_score = pred_patch.reshape(pred_patch.shape[0], -1).sum(dim=1).flatten()
            idx_pos = torch.where(score> 0)[0].detach().cpu().numpy()
            idx_neg = torch.where(score == 0)[0].detach().cpu().numpy()
            ...
            if len(idx_pos) > len(idx_neg):
                ...
                idx_neg = idx_neg[:len(idx_pos)]
            ms3 = ms3.flatten()
            ms3 = (ms3[idx_pos] - ms3[idx_neg])
            labels3 = torch.ones_like(ms3)

            # del data
            l = self.loss(output, target)
            if len(labels1)>0:
                tmp = self.cls_loss(ms1, labels1)
                l += tmp
            if len(labels2)>0:
                tmp = self.cls_loss(ms2, labels2)
                l += 0.8 * tmp
            if len(labels3)>0:
                tmp = self.cls_loss(ms3, labels3)
                l += 0.64 * tmp
