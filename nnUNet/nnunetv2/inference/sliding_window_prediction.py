...
def maybe_mirror_and_predict(network: nn.Module, x: torch.Tensor, mirror_axes: Tuple[int, ...] = None) \
    ...
    return prediction
def predict_sam(network, sam, x):
    with torch.no_grad():
        img_slice = x.permute(0, 2, 1, 3, 4).reshape(-1, 1, 320, 320).repeat(1, 3, 1, 1)
        img_slice = F.pad(img_slice, (96, 96, 96, 96), mode="constant", value=-1.3)
        img_slice = F.interpolate(img_slice, mode="bilinear", scale_factor=2)
        feature_list = [[], [], [], []]
        for i in range(len(img_slice)):
            input_features = sam(img_slice[i: (i + 1)])
            for j in range(4):
                feature_list[j].append(input_features[j])
        feature_list = [torch.cat(i, dim=0).reshape(-1, 16, 64, 64, 768).permute(0, 4, 1, 2, 3) for i in
                        feature_list]
    del img_slice
    torch.cuda.empty_cache()
    prediction = network(x, feature_list)
    return prediction
def maybe_mirror_and_predict_sam(network: nn.Module, sam:nn.Module, x: torch.Tensor, mirror_axes: Tuple[int, ...] = None) \
        -> torch.Tensor:
    prediction = predict_sam(network, sam, x)
    ...
    if mirror_axes is not None:
        ...
        num_predictons = 2 ** len(mirror_axes)
        if 0 in mirror_axes:
            prediction += torch.flip(predict_sam(network, sam, torch.flip(x, (2,))), (2,))
        if 1 in mirror_axes:
            prediction += torch.flip(predict_sam(network, sam, torch.flip(x, (3,))), (3,))
        if 2 in mirror_axes:
            prediction += torch.flip(predict_sam(network, sam, torch.flip(x, (4,))), (4,))
        if 0 in mirror_axes and 1 in mirror_axes:
            prediction += torch.flip(predict_sam(network, sam, torch.flip(x, (2, 3))), (2, 3))
        if 0 in mirror_axes and 2 in mirror_axes:
            prediction += torch.flip(predict_sam(network, sam, torch.flip(x, (2, 4))), (2, 4))
        if 1 in mirror_axes and 2 in mirror_axes:
            prediction += torch.flip(predict_sam(network, sam, torch.flip(x, (3, 4))), (3, 4))
        if 0 in mirror_axes and 1 in mirror_axes and 2 in mirror_axes:
            prediction += torch.flip(predict_sam(network, sam, torch.flip(x, (2, 3, 4))), (2, 3, 4))
        prediction /= num_predictons
    return prediction
...
def predict_sliding_window_return_logits_sam(network: nn.Module, sam:nn.Module,
                                         input_image: Union[np.ndarray, torch.Tensor],
                                         num_segmentation_heads: int,
                                         tile_size: Union[Tuple[int, ...], List[int]],
                                         mirror_axes: Tuple[int, ...] = None,
    ...
    with torch.no_grad():
        ...
        with torch.autocast(device.type, enabled=True) if device.type == 'cuda' else dummy_context():
            ...
            for sl in slicers:
                ...
                prediction = maybe_mirror_and_predict_sam(network, sam, workon, mirror_axes)[0].to(results_device)
